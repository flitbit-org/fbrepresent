    ...
    public class JsonRepresentationTest
    {
        ...
        public void JsonRepresentation_CanRoundTripModelWithInterfaceDrivenArray()
        {
            ...
            for (var i = 0; i < 1000; i++)
            {
                ...
                if (dto.Items.Any())
                {
                    Assert.AreEqual(dto.Items.ElementAt(0).Id, restoredDto.Items.ElementAt(0).Id);
                    Assert.AreEqual(dto.Items.ElementAt(0).Name, restoredDto.Items.ElementAt(0).Name);
                    Assert.AreEqual(dto.Items.ElementAt(0).CreatedAt, restoredDto.Items.ElementAt(0).CreatedAt);

                    var lastIndex = dto.Items.Count() - 1;
                    Assert.AreEqual(dto.Items.ElementAt(lastIndex).Id, restoredDto.Items.ElementAt(lastIndex).Id);
                    Assert.AreEqual(dto.Items.ElementAt(lastIndex).Name, restoredDto.Items.ElementAt(lastIndex).Name);
                    Assert.AreEqual(dto.Items.ElementAt(lastIndex).CreatedAt, restoredDto.Items.ElementAt(lastIndex).CreatedAt);
                }
            }
        }
        ...
        IArrayBasedModel SetupArrayModel(IFactory factory)
        {
            ...
            arrayModel.Items = SetupItems(factory).ToList();
        ...
        {
            for (var i = 0; i < Random.Next(0, 15); i++)
            {
